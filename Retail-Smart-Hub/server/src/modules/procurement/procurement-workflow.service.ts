...
export function recalculatePurchaseOrderStatus(purchaseOrderId: string) {
  ...
  const hasArrivalInProgress = (db.prepare<{ count: number }>(`
    SELECT COUNT(*) as count
    FROM receiving_notes rn
    WHERE rn.purchase_order_id = ? AND rn.status <> ?
  `).get(purchaseOrderId, ARRIVAL_STATUS_DONE)?.count ?? 0) > 0;

  let nextStatus = PO_STATUS_ACTIVE;
  if (!hasArrived) {
    nextStatus = currentStatus === PO_STATUS_DRAFT ? PO_STATUS_DRAFT : PO_STATUS_ACTIVE;
  } else if (!hasUnreceived && !hasPendingInbound && !hasArrivalInProgress) {
