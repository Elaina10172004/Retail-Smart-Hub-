...
const WRITE_TOOL_SCHEMAS: Record<RuntimeWriteToolName, RuntimeToolSchemaDefinition> = {
  ...
  create_product_master_data: {
    ...
    parameters: {
      type: 'object',
      additionalProperties: false,
      required: ['sku', 'name', 'preferredSupplierName'],
      properties: {
        sku: { type: 'string' },
        name: { type: 'string' },
        ...
        costPrice: { type: 'number', minimum: 0.01, description: '进货价/成本价。采购场景下必须填写，销售场景下可选。' },
        salePrice: { type: 'number', minimum: 0.01, description: '销售价。仅在创建销售单相关商品时填写，采购场景下可留空。' },
    ...
    parser: z
      .object({
        ...
        unit: z.string().trim().optional(),
        safeStock: z.coerce.number().int().min(0).optional(),
        salePrice: z.coerce.number().gte(0).optional(),
